...
compatibility: Requires CMake and the project-local libmcp-project MCP server when available.
metadata:
  project: libmcp
  scope: cmake
---
...
2. Use `libmcp-project` MCP first. Begin with `git_status`, `list_cmake_presets`, and only relevant `project_tree` or `read_project_file` calls.
